...
- [ ] 뽀모도로 타이머 웹 푸시(FCM)
  - 모바일에선 화면이 자동으로 꺼지기 때문에 백그라운드로 돌아가게 되어 뽀모도로 모드에서 끝날때를 알려야함 (네이티브 앱이 아니라 백그라운드 모드 불가)
  - 웹 푸시를 허용하지 않으면 뽀모도로 모드를 사용하지 못하도록 막기
  - 뽀모도로 타이머용 api 작업 (서버에서 redis에 알람을 보낼 기기와 시간을 저장하여 배치작업을 통해 일정시간(10 ~ 30초?)마다 알림 발송 후 기록)
  - 뽀모도로 타이머 중단 시 redis에서 해당 작업 제거
...
- [ ] 앱 실행 후 간편 로그인을 했다면 `session storage`에 상태를 저장하여 뒤로가기 시 즉시 종료로 동작 시키기
  - redirect로 인한 종료 로직 오작동 해결 트릭
- [ ] 일반 타이머 모드 추가
- [ ] 뽀모도로 모드 웹 푸시 알림 추가(fcm)
