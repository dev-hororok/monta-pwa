...
앱이 종료될 때 중간에 다른 도메인이 껴있으면 해당 페이지로 이동함

##### 해결책

- [x] 1. history를 되도록 쌓지 않고 delay초 내에 뒤로가기가 한번 더 발생 시 for문을 통해 history.length만큼 back()
  ```ts
  for (let i = 0; i < window.history.length; ++i) {
    window.history.back();
  }
  ```
- 위 방법은 문제점을 어느정도 해결하지만 아래 문제가 해결되지 않음
  `자바스크립트로 history를 조작 한 후 사용자의 실제 상호작용(터치 등)이 없으면 pop 동작 시 현재 도메인의 히스토리가 몇개든 전체(같은 도메인)가 pop됨`

- 해결책 (2)
  - 간편로그인 `redirect` 동작을 전부 `popup` 방식으로 변경하여 외부 히스토리 방지하기
  - [ ] 앱 실행 후 간편 로그인을 했다면 `session storage`에 상태를 저장하여 뒤로가기 시 즉시 종료로 동작 시키기
        (처음만 위 for문 방식으로 종료시키고 이후 부턴 모바일 종료 방식)
